...
data_path = os.path.join(settings.BASE_DIR,'static','datos')
...
if os.name == 'nt':
    ...
    path_csv_sequences = 'C:\\Users\\catalina\\Documents\\Users_data\\sequences\\'
else:
...
def get_n_correct_features(a_matrix,limit):
    ...
    for i in range(limit):
        ...
        if a_matrix[the_index,i] == -1:
            ...
            correct_distance.append(a_matrix[the_index,i])
            correct_indexs.append(the_index)
            n_identified += 1
    return [n_identified,selected_distance,identified_indexs,abstenidos,correct_indexs,correct_distance]